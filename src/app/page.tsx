...
export default function HomePage() {
  return (
    <main className="relative overflow-hidden">
      ...
      {/* HERO */}
      <section className="container mx-auto max-w-6xl px-4 pt-12 pb-8 md:pt-16 md:pb-12">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-[#0f2250] to-[#1b2f66] p-6 md:p-8 ring-1 ring-white/10 shadow-[0_18px_45px_rgba(0,0,0,.35)]">
          <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-center">
            ...
            <div className="flex flex-wrap gap-3">
              <CTA href="https://t.me/UniHero_BOT" target="_blank" variant="primary">Chat with Bot 🤖</CTA>
              <CTA href="https://t.me/UniHero_admin" target="_blank" variant="ghost">Talk to Admin 👤</CTA>
            </div>
          </div>
        </div>
      </section>
...
function CTA({
  href, children, variant = "primary", target,
}: { href: string; children: ReactNode; variant?: "primary" | "ghost"; target?: "_blank" }) {
  const base =
  ...
  return <Link href={href} target={target} className={`${base} ${styles}`}>{children}</Link>;
}
